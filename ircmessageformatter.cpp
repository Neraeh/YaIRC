...
{
    if (!message.isEmpty()) {
        QString formatted = QObject::tr("<font color='gray'>[%1]</font> %2").arg(QTime::currentTime().toString(), message);
...
QString IrcMessageFormatter::formatPrivateMessage(IrcPrivateMessage* message)
{
    ...
    if (message->isAction())
        ...
        return QObject::tr("<font color='#a3a370'>%1:</font> %2").arg(message->nick(), content);
}
...
{
    return QObject::tr("! %1").arg(message->error());
}
